...
export class LerpData {
  ...
  speed: number = 1
  constructor(origin: Vector3, target: Vector3, downStreet: boolean, speed: number){
    this.origin = origin
    this.target = target
...
export function createWalker(path: string, clipName: string, up: boolean, speed: number){

    let walker = new Entity()
    walker.add(new GLTFShape(path))
    if (up){
      walker.add(new Transform({
        position: upL,
